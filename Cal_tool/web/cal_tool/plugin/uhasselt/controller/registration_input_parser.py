...
class UHasseltRegistrationInputParser(BaseRegistrationInputParser):
    def __init__(self):
        super().__init__()
    ...
    def check_mail_valid(p_email):
        ...
        # Add this to make uhasselt plugin only to UHasselt students: and re.match("^.+@([?)[a-zA-Z0-9-.])*\.?uhasselt\.be$", p_email) is not None
        if not mail_taken :
            return True
        return False
